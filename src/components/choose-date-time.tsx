...
export default function ChooseDateTime({
  ...
  return (
    <div className='container text-theme p-4 rounded-lg'>
      <div className='d-flex justify-content-between align-items-center mb-4'>
        <div className='small'>{formatDateRange(weekDates)}</div>
      ...
      <div className='row mb-4 flex-nowrap container-date-picker'>
        {weekDates.map((date, index) => (
          <div key={index} className='col text-center'>
            <div className='small mb-1'>{date.toLocaleDateString(locale, { weekday: 'short' })}</div>
            <button
      ...
      <h2 className='h5 mb-3'>{t('booking.choose_time')}</h2>
      <div className='d-flex justify-content-between align-items-center mb-3'>
        <div className='btn-group'>
          <button
            className={`btn ${timeOfDay === 'Morning' ? 'btn-primary' : 'bg-light-subtle text-theme'}`}
            onClick={() => setTimeOfDay('Morning')}
          >
            {t('booking.morning')}
          </button>
          <button
            className={`btn ${timeOfDay === 'Afternoon' ? 'btn-primary' : 'bg-light-subtle text-theme'}`}
      ...
      <h3 className='h5 mb-3'>{timeOfDay === 'Morning' ? t('booking.morning') : t('booking.afternoon')}</h3>
        ...
        {(timeOfDay === 'Morning' ? morningSlots : afternoonSlots).map(
          (slot) => (
            <div key={slot} className='col'>
              <button
                className={`btn btn-sm bg-light-subtle text-theme w-100 ${selectedTimeSlot === slot ? 'border border-warning' : ''}`}
